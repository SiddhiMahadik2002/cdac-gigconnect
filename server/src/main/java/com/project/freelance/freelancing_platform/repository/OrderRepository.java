...
public interface OrderRepository extends JpaRepository<Order, Long> {
        ...
        // Get recent orders for freelancer that contain client notes/feedback
        @Query("SELECT o FROM Order o WHERE o.freelancer.freelancerId = :freelancerId AND o.clientNotes IS NOT NULL ORDER BY o.completedAt DESC NULLS LAST")
        java.util.List<Order> findTopRecentWithClientNotesByFreelancerId(@Param("freelancerId") Long freelancerId);
        ...
        // Sum of amounts for a freelancer filtered by status (returns 0 when no rows)
        @Query("SELECT COALESCE(SUM(o.amount), 0) FROM Order o WHERE o.freelancer.freelancerId = :freelancerId AND o.status = :status")
        java.math.BigDecimal sumAmountByFreelancerIdAndStatus(@Param("freelancerId") Long freelancerId,
                        @Param("status") Order.OrderStatus status);
}